...
\entryDated
  ...
  \bl
    ...
    \item \textbf{YaraGen} (\textbf{Booz Allen Hamilton}--sponsored) --- end-to-end LLM system for automated YARA threat-detection rule generation \textit{(USENIX Security 2027, submitted)}:
    \sbl
    ...
    \sble
    \item \textbf{AutoPYara} --- open-sourced rule-generation package (PyPI) \textit{(ACSAC 2026, accepted)}:
    \sbl
...
\bl
  \item \textbf{YaraGen} --- USENIX Security 2027 (submitted).
  \item \textbf{AutoPYara} --- ACSAC 2026 (accepted); open-sourced on PyPI.
